       ...
       case when totalm=totali then true else false 
        ...
        function get_listado_filtro($where=null){
            ...
            if(isset($pd)){//pd solo tiene valor cuando el usuario esta asociado a un perfil de datos
             ...
             " (select distinct t_f.id_form,t_b.nombre||' Nro Cuenta: '||t_cu.nro_cuenta as disponibilidad,t_f.fecha_envio,t_f.id_origen_recurso,t_f.id_programa,lpad(cast(t_f.id_programa as text),2,'0') as prog,t_f.ano_cobro,t_f.anio_ingreso,extract(year from t_f.fecha_creacion) as anio_creacion,t_f.nro_ingreso,t_f.nro_expediente,t_f.fecha_creacion,t_f.id_dependencia,t_f.id_recibo,t_f.check_presupuesto,t_f.observacionpresupuesto,observacionfinanzas,case when (t_f.id_dependencia='FAIN' and t_f.id_origen_recurso in (1,3)) then case when t_f.nro_ingreso is not null then 'SI' else 'NO' end else case when t_f.pasado_pilaga then 'SI' else 'NO' end end  as pasado_pilaga,case when (t_f.id_dependencia='FAIN' and t_f.id_origen_recurso in (1,3)) then case when t_f.nro_ingreso is not null then 1 else 0 end else case when t_f.pasado_pilaga then 1 else 0 end end  as pas_pilaga,case when t_f.id_punto_venta<=0 then true else false end as sin_facturacion, lpad(cast(t_f.nro_ingreso as text),4,'0')||'/'||t_f.anio_ingreso as numero_ingreso,t_f.id_punto_venta, case when t_f.id_punto_venta<=0 then 0 else t_f.id_punto_venta end as pv,t_p.descripcion as desc_pv,t_f.estado,t_c.titulo as origen ,t_t.total as monto,case when archivo_form is null then 0 else 1 end as archivo, case when t_f.id_recibo is null then 0 else case when t_r.archivo_recibo is null then 1 else 0 end end as archivo_recib,destino_sueldo "
                      ." from formulario t_f 
