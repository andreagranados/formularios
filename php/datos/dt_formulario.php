        ...
        function get_listado_filtro($where=null){
          ...
          // print_r($resul);
            ...
            $sql="select *, case when check_presupuesto then 'SI' else 'NO' end as check_pres from 
                (select t_f.id_form,t_f.fecha_envio,t_f.id_origen_recurso,t_f.id_programa,t_f.mes_cobro||'/'||t_f.ano_cobro as cobrado,t_f.anio_ingreso,extract(year from t_f.fecha_creacion) as anio_creacion,t_f.nro_ingreso,t_f.nro_expediente,t_f.fecha_creacion,t_f.id_dependencia,t_f.id_recibo,t_f.check_presupuesto,t_f.observacionpresupuesto,observacionfinanzas,pasado_pilaga,case when t_f.id_punto_venta<=0 then true else false end as sin_facturacion, t_f.nro_ingreso||'/'||t_f.anio_ingreso as numero_ingreso, case when t_f.id_punto_venta<=0 then 0 else t_f.id_punto_venta end as id_punto_venta,t_f.estado,t_c.titulo as origen ,sum(t_i.monto) as monto
                         from formulario t_f 
